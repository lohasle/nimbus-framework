package com.nimbus.module.infra.service.codegen.inner;
...
import com.nimbus.framework.apilog.core.annotation.ApiAccessLog;
import com.nimbus.framework.apilog.core.enums.OperateTypeEnum;
...
import com.nimbus.framework.common.util.date.DateUtils;
import com.nimbus.framework.common.util.date.LocalDateTimeUtils;
import com.nimbus.framework.common.util.object.BeanUtils;
import com.nimbus.framework.common.util.object.ObjectUtils;
import com.nimbus.framework.common.util.string.StrUtils;
import com.nimbus.framework.excel.core.annotations.DictFormat;
import com.nimbus.framework.excel.core.convert.DictConvert;
import com.nimbus.framework.excel.core.util.ExcelUtils;
...
import com.nimbus.module.infra.enums.codegen.CodegenVOTypeEnum;
...
import static com.nimbus.framework.common.exception.util.ServiceExceptionUtil.exception;
import static com.nimbus.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static com.nimbus.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 nimbus-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 nimbus-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.nimbus.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/nimbus-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/nimbus-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "nimbus-module-${table.moduleName}/" + // 顶级模块
                "nimbus-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "nimbus-module-${table.moduleName}/" + // 顶级模块
                "nimbus-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "nimbus-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "nimbus-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "nimbus-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "nimbus-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
