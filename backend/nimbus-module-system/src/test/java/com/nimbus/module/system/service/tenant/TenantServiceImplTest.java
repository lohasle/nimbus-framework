package com.nimbus.module.system.service.tenant;

import com.nimbus.framework.common.enums.CommonStatusEnum;
import com.nimbus.framework.common.pojo.PageResult;
import com.nimbus.framework.tenant.config.TenantProperties;
import com.nimbus.framework.tenant.core.context.TenantContextHolder;
import com.nimbus.framework.test.core.ut.BaseDbUnitTest;
import com.nimbus.module.system.controller.admin.tenant.vo.tenant.TenantPageReqVO;
import com.nimbus.module.system.controller.admin.tenant.vo.tenant.TenantSaveReqVO;
import com.nimbus.module.system.dal.dataobject.permission.MenuDO;
...
import com.nimbus.module.system.dal.dataobject.tenant.TenantPackageDO;
import com.nimbus.module.system.dal.mysql.tenant.TenantMapper;
import com.nimbus.module.system.enums.permission.RoleCodeEnum;
import com.nimbus.module.system.enums.permission.RoleTypeEnum;
import com.nimbus.module.system.service.permission.MenuService;
...
import com.nimbus.module.system.service.tenant.handler.TenantInfoHandler;
import com.nimbus.module.system.service.tenant.handler.TenantMenuHandler;
import com.nimbus.module.system.service.user.AdminUserService;
...
import static com.nimbus.module.system.dal.dataobject.tenant.TenantDO.PACKAGE_ID_SYSTEM;
...
public class TenantServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateTenant() {
        ...
        when(userService.createUser(argThat(user -> {
            assertEquals("nimbus", user.getUsername());
            assertEquals("sample", user.getPassword());
            assertEquals("Nimbus", user.getNickname());
        ...
        TenantSaveReqVO reqVO = randomPojo(TenantSaveReqVO.class, o -> {
            o.setContactName("Nimbus");
            o.setContactMobile("15601691300");
            o.setPackageId(100L);
            o.setStatus(randomCommonStatus());
            o.setWebsites(singletonList("https://www.nimbus.local"));
            o.setUsername("nimbus");
            o.setPassword("sample");
    ...
    public void testGetTenantPage() {
        ...
        reqVO.setName("Nimbus");
        reqVO.setContactName("imb");
    ...
    public void testGetTenantByName() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setName("Nimbus"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByName("Nimbus");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
    ...
    public void testGetTenantByWebsite() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setWebsites(singletonList("https://www.nimbus.local")));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByWebsite("https://www.nimbus.local");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
